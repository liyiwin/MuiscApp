...
class SearchResultRequestConverter:RequestResultConverter() {

    private val gson = Gson()

    fun convertSearchTrackOnResponse(response: Response<ResponseBody>): RequestResultWithData<List<Track>> {
        ...
        if(responseCode >= 300 ){
            ...
            try{
                ...
                if(total == 0) return RequestResultWithData.Success("已到達最後一頁", listOf())
    ...
    fun convertSearchTrackOnFailure(call: Call<ResponseBody>, t: Throwable): RequestResultWithData<List<Track>>{
