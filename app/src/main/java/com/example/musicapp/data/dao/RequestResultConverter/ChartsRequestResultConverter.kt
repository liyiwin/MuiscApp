...
class ChartsRequestResultConverter :RequestResultConverter(){
    ...
    fun ConvertGetTotalChartsOnResponse(response: Response<ResponseBody>): RequestResultWithData<List<PlayList>> {
    ...
    fun ConvertGetTotalChartsOnFailure(call: Call<ResponseBody>, t: Throwable):RequestResultWithData<List<PlayList>>{
        val failureMessage = ConvertResponseFailureMessage(call,t)
        return RequestResultWithData.Failure(failureMessage)
    }

    fun ConvertGetChartOnResponse(response: Response<ResponseBody>): RequestResultWithData<PlayListContent>{
        val responseCode = response.code();
        if(responseCode >= 300 ){
            val errorMessage = ConvertResponseErrorMessage(response)
            return RequestResultWithData.Error(errorMessage)
    ...
    fun ConvertGetChartOnFailure(call: Call<ResponseBody>, t: Throwable):RequestResultWithData<PlayListContent>{
        val failureMessage = ConvertResponseFailureMessage(call,t)
        return RequestResultWithData.Failure(failureMessage)
    }
